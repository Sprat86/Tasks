...
boyAndCookie(21, 95, 58); // "В ходе соревнований ни одна печенюшка не пострадала"
boyAndCookie(22, 75, 70); // "Правом первым откусить печенюшку обладает boy B"
boyAndCookie(33, 86, 59); // "Правом первым откусить печенюшку обладает boy A"




/** Задача:
Мария с Иваном договорились пообедать в кафе. Также они договорились разделить счет пополам, но с одним условием: если среди заказанных блюд есть то,
 на которое у Марии аллергия - она его не оплачивает.
 1. стоимость блюд представлена массивом целых чисел 'bill';
 2. блюдо, на которое у Марии аллергия представлена аргументом функции 'dish' и обозначает позицию блюда в чеке;
 3. сумма, которую Мария внесла на счет представлена аргументом функции 'pay'.
 Например:
Счет: [2, 4, 8]. У Марии аллергия на блюдо под номером arr.[dish]=2, стоимостью 8 монет. Мария должна внести на счет (2 + 4)/2 = 3. Если же Иван обсчитался и взял
 с Марии 7 монет - (2 + 4 + 8)/2, то он должен вернуть ей обратно 4 монеты.
 Если 'pay' = 7, то функция должна вернуть фразу "Иван должен вернуть Марии 4 монеты." - разницу между тем, что внесла Мария и тем, что должна была внести.
 Если 'pay' = 3, то функция должна вернуть фразу "Bon Appetit".
 *
 */
let bill = [3, 10, 2, 9];
let dish = 1;
...
function bonAppetit(bill, dish, pay) {

    bill.splice(dish, 1);

    let summa = bill.reduce((sum, item) => {return sum + item;});

    if(summa / 2 < pay) {
        return "Иван должен вернуть Марии " + (pay - summa/2) + " монет(ы).";
    } else if (summa / 2 === pay) {
        return 'Bon Appetit!';
    }
}

bonAppetit(bill, dish, pay);
// "Иван должен вернуть Марии 5 монет(ы)."
// "Bon Appetit!"
